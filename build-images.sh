...
# Reuse existing nodebuilder-chiefonboarding container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-chiefonboarding; then
	echo "Pulling NodeJS runtime..."
	buildah from --name nodebuilder-chiefonboarding -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
	...
	--label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/chiefonboarding/chiefonboarding:${APP_VERSION}" \
	"${container}"
# Commit the image
